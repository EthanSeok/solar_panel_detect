...
class Labelme2YOLO(object):
    ...
    def _make_train_val_dir(self):
        ...
        paths = [os.path.join(self._label_dir_path + 'train/'), os.path.join(self._label_dir_path + 'val/'),
                 os.path.join(self._image_dir_path + 'train/'), os.path.join(self._image_dir_path + 'val/')]
        for yolo_path in paths:
    ...
    def _get_label_id_map(self, json_dir):
        ...
        return OrderedDict([(label, label_id) for label_id, label in enumerate(sorted(label_set))])
    ...
    def convert(self, val_size):
        ...
        folders = [file_name for file_name in os.listdir(self._json_dir) if os.path.isdir(os.path.join(self._json_dir, file_name))]
    ...
    def _get_circle_shape_yolo_object(self, shape, img_h, img_w):
        ...
        obj_w = 2 * radius
        obj_h = 2 * radius
        yolo_center_x = clip(obj_center_x / img_w)
        yolo_center_y = clip(obj_center_y / img_h)
        yolo_w = clip(obj_w / img_w)
        yolo_h = clip(obj_h / img_h)
    ...
    def _get_other_shape_yolo_object(self, shape, img_h, img_w):
        ...
        yolo_center_x = clip((obj_x_min + obj_w / 2.0) / img_w)
        yolo_center_y = clip((obj_y_min + obj_h / 2.0) / img_h)
        yolo_w = clip(obj_w / img_w)
        yolo_h = clip(obj_h / img_h)
        return label_id, yolo_center_x, yolo_center_y, yolo_w, yolo_h
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_size', type=float, nargs='?', default=0.2, help='Please input the validation dataset size, for example 0.1')
    parser.add_argument('--json_name', type=str, nargs='?', default=None, help='If you put json name, it would convert only one json file to YOLO.')
